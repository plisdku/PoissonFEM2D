...
Wd2 = 164e-3;
        ...
        geom2d.addContour(@(p) [-Lx+e_d+Wd+l1+L1+l2+L1+l2, -Lx+e_d+Wd+l1+L1+l2+L1+l3+l2, -Lx+e_d+Wd+l1+L1+l2+L1+l3+l2, -Lx+e_d+Wd+l1+L1+l2+L1+l2], @(p) [D1, D1, Ly-d, Ly-d], s*ratio*[1, 1, 2, 2], 4, 1:4);
...
fn_handle = @(fem) @(p) return_FEM_function_chrom(fem, p, Lx, Ly); 
minX = ones(1,3)'*(-25);
...
[x_voltages, fval, iter] = extremize12(fn_handle(fem), x0, 'Bounds', [minX, maxX], 'Callback', @ plotfunc_voltage_chrom);
