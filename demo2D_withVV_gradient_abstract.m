...
for nn = 1:length(deltas)
    ...
    [F, DF, xv_all, DFDEr, E_x] = ElectronSetupdemo2D_abst(femProblem.uCartesian, xCoarse, yCoarse, measBox, measNxy); 
   % [F, DF, xv_all, DFDEr, E_x] = ElectronSetup_obj(femProblem.uCartesian, xCoarse, yCoarse, measBox, measNxy); 
figure(1); clf
   ...
   % quiver(xyGeomNodes(id,1), xyGeomNodes(id,2), femProblem.dF_dxy(id,1), femProblem.dF_dxy(id,2), 'g-', 'linewidth', 1)
    ...
    for ii = 1:size(xv_all,2)
            ...
            plot(xv_all(ix_x(1),ii),xv_all(ix_y(1),ii),'rx')
    end
