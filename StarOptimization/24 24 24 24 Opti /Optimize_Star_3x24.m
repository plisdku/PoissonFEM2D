...
% 
% %r1_start = r_alpha1*ones(1,8);
% %r2_start = r_alpha2*ones(1,8);
...
%s1_vec = s_vec_lin([7 7 7 7],2,1);
%s2_vec = s_vec_lin([7 7 7 7],2,1);
%s3_vec = s_vec_lin([7 7 7 7],2,1);
...
%[minX, maxX] = getBoundsStar([5 5 5], r1_start, r_i);
minX = [ones(1,24)'*(-r1_start(1)); -5e-3; -r_i; -pi/24*ones(1,24)';ones(1,24)'*(-r2_start(1)); -5e-3; -r_i; -pi/24*ones(1,24)';ones(1,24)'*(-r3_start(1)); -5e-3; -r_i; -pi/24*ones(1,24)'];
%minX = ones(1,end_p3)'*(-5e-3);
%maxX = ones(1,end_p3)'*(10e-3);
maxX = [ones(1,24)'*(r1_start(1)); 5e-3; 10*r_i; pi/24*ones(1,24)';ones(1,24)'*(r2_start(1)); 5e-3; 10*r_i; pi/24*ones(1,24)';ones(1,24)'*(r3_start(1)); 5e-3; 10*r_i; pi/24*ones(1,24)'];
t1 = tic;

plot_func = @(xHistory,fHistory,DfHistory,max_x, alpha) plotfunc_star_alpha(xHistory, fHistory, DfHistory, max_x, alpha, end_p3, [5 5 5], 1);
...
save('StarOpti3x24' , 'x' , 'fval', 'iter', 'xHist', 'fHist', 'DfHist', 'alphas')
% 
