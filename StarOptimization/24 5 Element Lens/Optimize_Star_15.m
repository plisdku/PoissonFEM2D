...
% [angles5_start, r5_start, s5_vec] = getInitialVectorsStarBox(L4,r_i,r_o,5);
% 
...
% x4_vec = @(p) r4_start.*cos(angles4_start) + center4_start(1);
% x5_vec = @(p) r5_start.*cos(angles5_start) + center5_start(1);
% 
...
% y4_vec = @(p) r4_start.*sin(angles4_start) + center4_start(2);
% y5_vec = @(p) r5_start.*sin(angles5_start) + center5_start(2);
% 
% 
...
% geom2d.addContour(x4_vec, y4_vec, s*ratio*s3_vec, 5, 1:l4);
% geom2d.addContour(x5_vec, y5_vec, s*ratio*s3_vec, 6, 1:l5);
% 
% safetyplotGeom(geom2d, zeros(1,5))
% 
...
% fem.setDirichlet(5, @(p,x,y) 0 + p(4));
% fem.setDirichlet(6, @(p,x,y) 0 + p(5));
% 
...
% 
% 
% fn_handle = @(fem) @(p) return_FEM_function_new(fem, p, Lx_outer, Ly); 
% %[minX, maxX] = getBoundsStar_voltage_5elem([5 5 5], r1_start, r2_start, r3_start, r_i);
% minX = -50e3*ones(1,5)';
% maxX = 50e3*ones(1,5)';
% t1 = tic;
% 
...
% toc(t1)
% save('StarOpti_fiveElement_sequential_voltage' , 'x' , 'fval', 'iter', 'xHist', 'fHist', 'DfHist', 'alphas')
% % 
% Vs = x;
% %%
% geom2d = PoissonFEM2D.ParameterizedGeometry2D();
...
% 
% safetyplotGeom(geom2d, zeros(1,end_p5))
% 
% fem = PoissonFEM2D.FEMInterface(geom2d, N_field, N_geom, N_quad, isAxisymmetric);
% fem.setNeumann(1, @(p,x,y) 0.0);
% 
% fem.setDirichlet(2, @(p,x,y) 0 + Vs(1));
% fem.setDirichlet(3, @(p,x,y) Vb + Vs(2));
% fem.setDirichlet(4, @(p,x,y) 0 + Vs(3));
% fem.setDirichlet(5, @(p,x,y) 0 + Vs(4));
% fem.setDirichlet(6, @(p,x,y) 0 + Vs(5));
% 
% fem.setFreeCharge(@(p,x,y) 0.0);
% 
% 
...
% 
% fn_handle = @(fem) @(p) return_FEM_function_new(fem, p, Lx_outer, Ly); 
% [minX, maxX] = getBoundsStar_shape_5elem([5 5 5], r1_start, r2_start, r3_start, r_i);
% 
% t1 = tic;
% plot_func = @(xHistory,fHistory,DfHistory,max_x,alpha) plotfunc_star_alpha4(xHistory, fHistory, DfHistory, max_x, alpha, end_p3, [5 5 5], 1);
% 
% 
% %plot_func = @(xHistory,fHistory,DfHistory,max_x, alpha) plotfunc_star_voltage_alpha(xHistory, fHistory, DfHistory, max_x, alpha, end_p3, [5 5 5], 5);
% 
...
% toc(t1)
% save('StarOpti_fiveElement_shapeOnly' , 'x' , 'fval', 'iter', 'xHist', 'fHist', 'DfHist', 'alphas')
% 
...
% 
% 
% fn_handle = @(fem) @(p) return_FEM_function_45(fem, p, Lx_outer, Ly); 
% %[minX, maxX] = getBoundsStar_voltage_5elem([5 5 5], r1_start, r2_start, r3_start, r_i);
% minX = -50e3*ones(1,5)';
% maxX = 50e3*ones(1,5)';
% t1 = tic;
...
% %plot_func = @(xHistory,fHistory,DfHistory,max_x, alpha) plotfunc_star_voltage_alpha(xHistory, fHistory, DfHistory, max_x, alpha, end_p3, [5 5 5], 5);
% 
% [x, fval, iter, xHist, fHist, DfHist, times, alphas] = extremize_lars(fn_handle(fem), x0, 'Bounds', [minX, maxX], 'Callback', @ plotfunc_just_voltage4, 'MaxIter', 25);
% toc(t1)
% save('StarOpti_fiveElement_sequential_voltagechrom' , 'x' , 'fval', 'iter', 'xHist', 'fHist', 'DfHist', 'alphas')
% % 
% Vs = x;
%%
geom2d = PoissonFEM2D.ParameterizedGeometry2D();
...
t1 = tic;
plot_func = @(xHistory,fHistory,DfHistory,max_x,alpha) plotfunc_star_alpha44(xHistory, fHistory, DfHistory, max_x, alpha, end_p3, [5 5 5], 1);
...
[x, fval, iter, xHist, fHist, DfHist, times, alphas] = extremize_lars(fn_handle(fem), x0, 'Bounds', [minX, maxX], 'Callback', plot_func, 'MaxIter', 25);
toc(t1)
save('StarOpti_fiveElement_shapeOnlychrom' , 'x' , 'fval', 'iter', 'xHist', 'fHist', 'DfHist', 'alphas')
