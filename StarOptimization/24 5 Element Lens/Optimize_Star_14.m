...
fem.setDirichlet(2, @(p,x,y) 0 + p(end_p5 + 2));
fem.setDirichlet(3, @(p,x,y) Vb + p(end_p5 + 1));
fem.setDirichlet(4, @(p,x,y) 0 + p(end_p5 + 3));
fem.setDirichlet(5, @(p,x,y) 0 + p(end_p5 + 4));
fem.setDirichlet(6, @(p,x,y) 0 + p(end_p5 + 5));
...
toc(t1)
save('StarOpti_fiveElement3' , 'x' , 'fval', 'iter', 'xHist', 'fHist', 'DfHist', 'alphas')
% 
