...
r1_start = (r1_start./r1_start)*mean(r1_start)*0.8;
r2_start = r1_start;%(r2_start./r1_start)*mean(r1_start)*0.8;
r3_start = r1_start;%(r1_start./r1_start)*mean(r1_start)*0.8;
angles1_start = linspace(0,2*pi, length(angles1_start)+1);
angles1_start = angles1_start(1:(end-1));
angles2_start = angles1_start;
...
fn_handle = @(fem) @(p) return_FEM_function_star(fem, p, Lx_outer, Ly); 
...
[x, fval, iter, xHist, fHist, DfHist] = extremize_lars(fn_handle(fem), x0, 'Bounds', [minX, maxX], 'Callback', plot_func, 'MaxIter', 25);
toc(t1)
save('StarOpti24ShapeOnlyRoundInitial' , 'x' , 'fval', 'iter', 'xHist', 'fHist', 'DfHist')
% 
